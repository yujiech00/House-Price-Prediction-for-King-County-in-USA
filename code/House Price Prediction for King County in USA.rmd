---
title: "House Price Prediction for King County in USA"
...
```{r More data visualizations for demonstration purposes}
# Get numeric columns
...
```{r More data visualizations for region, also for demonstration purposes}
...
```{r remove data points from minor regions and remove columns that will not be used in the model}
...
```{r feature selection - stepwise AIC}
...
```{r diagonostic plots for checking whether the assumption of homogeneity holds or not}
...
```{r check by fitting a multiple regression model}
...
```{r save kv_house_data_family_filtered, kv_house_data_family_filtered_after_feature_selection as .rds}
...
``` {r notes for the following functions for model fitting and performance evaluation}
...
```{r import the cleaned and transformed dataset from saved .rds files}
...
```{r apply Cross Validation (CV) - main function}
...
```{r import model performance outputs from model_cross_validation_output.rds saved in order to generate the performance tables in the following sections}
...
``` {r Covert and save this R markdown file as an R script}
knitr::purl("/Users/yujiech/Documents/GitHub/House-Price-Prediction-for-King-County-in-USA/code/House Price Prediction for King County in USA.rmd")
```
